...
(My GitHub Repository)[https://github.com/T-Clarkson/BIOL3207_workshop]
...
## The files are not automatically backed up by GitHub or Git. You must push them into the cloud before GitHub will save any changes. This means that if you lose your progress for whatever reason, such as your computer crashing, GitHub will only have the changes up to the point from which you cloned or pulled the repository.
...
# echo = FALSE would instruct the file to only display the output of the code, not the code itself. As this section of the file is only text and has no output, it would not be displayed at all if echo was set to FALSE.
```
...
path <- "./output/data/OA_activitydat_20190302_BIOL3207_tidied.csv"
...
#Attempt 1: data <- data %>% select(-c(loc,comment)) ### - This was my original method, but this line is preventing knitting to HTML.
#Attempt 2: data <- data[,!names(data) %in% c("loc", "comment")]
#Attempt 3: data <- data
#           data$loc = NULL
#           data$comment = NULL
#Attempt 3 below:
...
list(unique(data$species))
list(unique(data$treatment))

data %>%
  group_by(species) %>%
  summarise(mean_sl = mean(sl),sd_sl=sd(sl),mean_activity=mean(activity),sd_activity=sd(activity),n=n())
...
## The red indicates code/text that has been removed and green indicates code/text that has been added.
```
...
```{r, prettyfig, fig.align='center', fig.cap="Box plots showing the means and ranges of the activity levels of fish of different species when exposed to different treatments."}
# ggplot figure showing mean activity for each treatment (Control, OA) for each species.
ggplot(data, mapping = aes(x = treatment,y = activity,colour=species)) +
  geom_boxplot()
...
ggsave('./output/figures/activity_by_species_and_treatmentss.png',width=6,height=3)
```
...
```{r}
#It should prevent GitHub from tracking changes in that pathway, so it doesn't stage/commit the figure.
```
...
```{r, answer6}
##Yes, because the coding for the figure would involve those images. If the images were not available to others via the repository, they would not be able to replicate the figure themselves.
```
...
```{r, collabFig, fig.cap = TRUE, echo=TRUE, eval=TRUE}
...
data %>% filter(species == c("chromis", "lemon", "acantho")) %>% ggplot(data2, mapping = aes(x = treatment,y = activity, fill=species)) +
...
```{r}
#We both added a line saying Hello/Bye at line 200. When the second person pushed their changes, they had a conflict arise. One of the options was then selected and remained. (We did this activity using the R Markdown files, not the README file)
```
...
```{r}
model<-lm(activity~treatment*species, data=data)
summary(model)
#Suggests a significant impact of treatment on activity of humbug and lemon fish.

model2<-lm(sl~treatment*species, data=data)
summary(model2)
#Shows no significant impact of treatment on the length of any species.

t.test(data$activity[data2$treatment=="CO2" & data$species=="humbug"],data$activity[data$treatment=="control" & data2$species=="humbug"])
#There is a significant difference between the activity levels of humbug fish exposed to control and high CO2 conditions (t-statistic = 2, df = 124, P=0.02, confidence intervals = 0.83, 9.9, difference in means = 5).

t.test(data$activity[data$treatment=="CO2" & data$species=="lemon"],data$activity[data$treatment=="control" & data$species=="lemon"])
#There is no significant difference in the activity levels of lemon fish exposed to control and high CO2 conditions (t-statistic = 2, df = 42, P=0.07, confidence intervals = -0.55, 13.04, difference in means = 6)

ggplot(data, aes(x=treatment,y=activity,colour=species))+
  geom_boxplot()
