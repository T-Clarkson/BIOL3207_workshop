...
### data2 <- data3 %>% select(-c(loc,comment)) ### - This was my original method, but this line is preventing knitting to HTML.
data2 <- data
data2$loc = NULL
data2$comment = NULL
# Check spelling in species and treatment but also generate a summary table
list(unique(data2$species))
list(unique(data2$treatment))
...
ggplot(data2, mapping = aes(x = treatment,y = activity,colour=species)) +
  geom_boxplot()
...
data2 %>% filter(species == c("chromis", "lemon", "acantho")) %>% ggplot(data2, mapping = aes(x = treatment,y = activity, fill=species)) +
...
```{r}
model<-lm(activity~treatment*species, data=data2)
summary(model)
#Suggests a significant impact of treatment on activity of humbug and lemon fish.

model2<-lm(sl~treatment*species, data=data2)
summary(model2)
#Shows no significant impact of treatment on the length of any species.

t.test(data2$activity[data2$treatment=="CO2" & data2$species=="humbug"],data2$activity[data2$treatment=="control" & data2$species=="humbug"])
...
t.test(data2$activity[data2$treatment=="CO2" & data2$species=="lemon"],data2$activity[data2$treatment=="control" & data2$species=="lemon"])
...
ggplot(data2, aes(x=treatment,y=activity,colour=species))+
  geom_boxplot()
